"""
vector_store.py  (shared_rag)
──────────────────────────────
ChromaDB client manager shared across all Jatayu agents.
Moved from et_service/monitoring_agent/rag/vector_store.py.
All collection logic and public API remain identical.

Manages five persistent collections:
  L1_regulatory      — text docs  → SentenceTransformer (384-d)
  L2_fraud_cases     — feature vecs → FraudFeatureEncoder (128-d, bootstrap)
  L3_typologies      — signal vecs  → SignalSequenceEncoder (256-d, bootstrap)
  L4_dynamic_weights — feature vecs → FraudFeatureEncoder (128-d, bootstrap)
...
def query_by_text(collection_name: str,
    ...
    """Text-similarity query — L1_regulatory ONLY."""
...
def query_by_vector(collection_name: str,
                    ...
                    n_results: int = 5) -> dict:
    """Pre-computed vector query — L2, L3, L4 ONLY."""
...
def get_by_id(collection_name: str, doc_id: str) -> dict | None:
    """Retrieves a single L5 record by its ID. Returns None if not found."""
    try:
        col = _get_collection(collection_name)
...
def _query(collection_name: str,
    ...
    """Shared query implementation. Handles empty collections and n_results clamping."""
    try:
        col = _get_collection(collection_name)
        count = col.count()
